...
public class Player : MonoBehaviour, ICanTakeDamage {
	...
	void Start() {
		...
		GameManager.Instance.Bullet = 100;
...
//		print ("Gravity: " + gravity + "  Jump Velocity: " + maxJumpVelocity);
    ...
    private void Dig()
    {
		Vector2 digPosition = transform.position;
		Vector2 digDirection = Vector2.down;

		RaycastHit2D hit = Physics2D.Raycast(digPosition, digDirection, digDepth, groundLayer);

		if (hit.collider != null)
		{
			// Emit particles at the hit point
			//digParticles.transform.position = hit.point;
			//digParticles.Play();

			// Destroy or modify the terrain here
...
//		controller.SetForce(force);
    ...
    //     {
    //rb.AddForce(Vector2.right*10000, ForceMode2D.Impulse);
    //canMove = false;
    //Debug.Log("boosted");
